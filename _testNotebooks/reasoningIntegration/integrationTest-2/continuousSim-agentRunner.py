...
# MAGIC
# MAGIC Runs the simulation in continuous (infinite or cyclic) mode with a
# MAGIC configurable agent. Agent selection is controlled by a single parameter
# MAGIC at the top of the notebook - no other cell is aware of which agent was chosen.
# MAGIC
# MAGIC **Stop the simulation** by clicking **Interrupt** in the notebook toolbar.
# MAGIC `ContinuousRunner` catches the interruption, writes `SIM_ENDED`, and prints
# MAGIC a final summary before exiting cleanly.
# MAGIC
# MAGIC **Run simultaneously with**:
# MAGIC [`continuousSim-liveDashboard.py`](./continuousSim-liveDashboard.py) - open
# MAGIC in a separate tab, pointing at the same `SIM_ID`, to watch the simulation
# MAGIC in real time. The dashboard notebook has no dependency on this one beyond the
# MAGIC shared `SIM_ID` and the Delta tables it writes to.
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC **Depends on**: Stages 1-7 complete (env tables exist, `ContinuousRunner`
# MAGIC available). World is built fresh in Section 4 below.
...
# MAGIC
# MAGIC **Note on `TICK_DURATION_SECONDS`**: the dashboard notebook polls at a
# MAGIC configurable interval. For a live feed, set `TICK_DURATION_SECONDS` to at
# MAGIC least 2–3 seconds so the dashboard sees meaningful new data on each poll.
# MAGIC `None` (full speed) will produce large bursts of ticks per dashboard poll
# MAGIC rather than a continuous stream.
...
# Inserted unconditionally here so the agent selection cell can import
# freely without a conditional sys.path block of its own.
# The import itself is deferred to the agent selection cell.
...
# MAGIC
# MAGIC Deletes all rows written by a previous run of this `SIM_ID`. Idempotent:
# MAGIC safe to re-run if this is a fresh `SIM_ID` with no prior data.
# MAGIC
# MAGIC Append-only tables are temporarily unlocked for the delete and re-locked
# MAGIC immediately after - same pattern used in the integration test notebook.
...
# MAGIC - Widget A: 5-tick cyclic custom demand with noise; stochastic demand spike
# MAGIC   from tick 10 onward (40% chance per tick) - tests agent response to
# MAGIC   unpredictable surge
...
# MAGIC - Probabilistic lead times (non-zero variability) to create realistic
# MAGIC   supply uncertainty
# MAGIC - No budget limit: continuous runs are not time-bounded so a hard budget
# MAGIC   cap would stop the simulation unpredictably
...
world_def = SimWorld(
    ...
    disruptions = [
        ...
        # 40% activation probability per tick - tests whether the agent
        # learns to anticipate surge rather than react to it.
        DisruptionSchedule(
        ...
        # 25% activation probability per tick - creates supply uncertainty
        # that a rule-based agent (which ignores lead time disruptions at
        # decision time) will handle worse than the LLM agent.
        DisruptionSchedule(
...
# MAGIC
# MAGIC This is the only cell that branches on `AGENT_TYPE`. Both branches
# MAGIC produce a single `agent` variable that implements `BaseAgent`. Everything
# MAGIC downstream (Section 7) is identical regardless of which branch was taken.
# MAGIC
# MAGIC **`"rule_based"`**: `RuleBasedAgent` - instantiated directly, no config needed.
# MAGIC
# MAGIC **`"llm"`**: `LLMAgentWrapper` with `LLMReorderAgent` as the internal executor.
# MAGIC Requires the LLM agent package on `sys.path` (set in Section 2). The wrapper
# MAGIC is the agent the runner sees; `LLMReorderAgent` is an internal detail of the
# MAGIC executor thread. See `warehouse_sim/agent/llm_agent_wrapper.py` for the full
# MAGIC design, and `devlog.md → Full LLM Agent Integration` for integration decisions.
...
if AGENT_TYPE == "rule_based":
    ...
    # See [DEP-4] in llm_agent_wrapper.py for the SparkSession injection rationale.
...
# MAGIC
# MAGIC Open [`continuousSim-liveDashboard.py`](./continuousSim-liveDashboard.py) in a
# MAGIC separate tab with the same `SIM_ID` to watch the simulation live while this
# MAGIC cell is running.
