...
# MAGIC ## 1. Parameters
# MAGIC
# MAGIC All configuration is loaded from `sim_config.yaml`. No values are hardcoded in this notebook. Edit the YAML to change any simulation or world parameter.
# MAGIC
# MAGIC **Shared keys** (`sim_id`, `catalog`): read by both the agent runner and the live dashboard. Changing them here changes them for both notebooks.
# MAGIC
