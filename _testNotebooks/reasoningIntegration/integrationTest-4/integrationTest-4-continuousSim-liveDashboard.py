...
# MAGIC [`integrationTest-4-continuousSim-agentRunner.py`](./integrationTest-4-continuousSim-agentRunner.py) in a separate tab. This notebook can be started before, during, or after the runner - it renders whatever data is in the tables at each poll interval.
# MAGIC
...
# MAGIC ## 1. Parameters
# MAGIC
# MAGIC `SIM_ID` and `CATALOG` are loaded from `sim_config.yaml` - the shared source of truth for both this notebook and the agent runner. They must not be set manually here.
# MAGIC
# MAGIC `POLL_INTERVAL_SECONDS` and `MAX_TICKS_TO_SHOW` are local to this notebook. They control polling and display behaviour only and have no bearing on the simulation itself.
